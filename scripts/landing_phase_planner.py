...
from scripts.SCP.scproblem_3dof_landing_1st_phase import SCProblem as SCProblem_3dof_1st
from scripts.SCP.scproblem_3dof_landing_phase_only import SCProblem as SCProblem_3dof
from scripts.SCP.scproblem_quad_6dof_socp_landing_phase_only import SCProblem as SCProblem_6dof
from scripts.SCP.scproblem_quad_6dof_socp_landing_1st_phase import SCProblem as SCProblem_6dof_1st
...
class scvx_traj():
    def __init__(self, quad_name, K):
        ...
        self.X = np.zeros(shape=[self.m.n_x, self.K_landing])
        self.U = np.zeros(shape=[self.m.n_u, self.K_landing])
        self.X, self.U = self.m.initialize_trajectory_landing_only(self.X, self.U, self.K_landing)
        ...
        # pre warm
        self.initial_p = np.array([4., 4.1, 2])
        ...
        A_bar_2, B_bar_2, C_bar_2, S_bar_2, z_bar_2 = self.integrator_landing.calculate_discretization(self.X[:, 0:self.K_landing], self.U[:, 0:self.K_landing], self.sigma_2, self.K_landing)
        ...
        self.problem.set_parameters(X_last=self.X, A_bar=A_bar_2, B_bar=B_bar_2, C_bar=C_bar_2, S_bar=S_bar_2, z_bar=z_bar_2,
                                    sigma_last_2=self.sigma_2, initial_p=self.initial_p,
...
class scvx_traj_1st():
    def __init__(self, quad_name, K):
        ...
        self.problem = SCProblem_6dof_1st(self.m, K)

        self.X = np.zeros(shape=[self.m.n_x, K_phase])
        self.U = np.zeros(shape=[self.m.n_u, K_phase])
        self.X, self.U = self.m.initialize_trajectory_landing_only(self.X, self.U, K_phase)
        ...
        self.problem.set_parameters(X_last=self.X, A_bar=A_bar_1, B_bar=B_bar_1, C_bar=C_bar_1, S_bar=S_bar_1, z_bar=z_bar_1,
                                    sigma_last_1=self.sigma_1, initial_p=self.initial_p,
                                    initial_v=self.initial_v, final_p=self.final_p, final_v=self.final_v,
                                    initial_q=self.initial_q, initial_w=self.initial_w, final_q=self.final_q, final_w=self.final_w,
                                    initial_u=self.initial_u, final_u=self.final_u,
                                    A=A,B=B,C=C)
...
class lcvx_traj_1st_phase():
    def __init__(self,quad_name, K):
        ...
        # self.K = K
        self.K = int(np.ceil(K/2))
        # START LCVX--------------------------------------------------------------------------------------

        self.problem = SCProblem_3dof_1st(self.m, self.K)
        ...
        self.safe_height = np.array([0, 0, 1]) # make sure endpoint of 1st phase is above platform

        self.problem.set_parameters(delta_t=(self.tf_guess_1 / (self.K - 1)), delta_t2=(self.tf_guess_1 / (self.K - 1)) ** 2, 
                                    initial_p=self.initial_p, initial_v=self.initial_v, final_p=self.final_p+self.safe_height, 
                                    final_v=self.final_v, initial_u=self.initial_u, final_u=self.final_u)
        error = self.problem.solve(verbose=False, solver=solver, ignore_dpp=False)
...
class lcvx_traj():
    def __init__(self,quad_name, K):
        ...
        self.problem = SCProblem_3dof(self.m, self.K)
        ...
        self.safe_height = np.array([0, 0, 1]) # initial position of 2nd phase

        self.problem.set_parameters(delta_t_2=(self.tf_guess_2 / (self.K - 1)), delta_t2_2=(self.tf_guess_2 / (self.K - 1)) ** 2, 
                                    initial_p=self.initial_p+self.safe_height,
                                    initial_v=self.initial_v, final_p=self.final_p, final_v=self.final_v,
                                    initial_u=self.initial_u, final_u=self.final_u)
        error = self.problem.solve(verbose=False, solver=solver, ignore_dpp=False)
...
def scvx_landing_phase_separated(quad_name, goal, tf_guess, init_p, plot=False, replan=False):
    ...
    # print(p_f, tf_guess)
    cvx_tracker = lcvx_traj_1st_phase(quad_name, K_3dof)
    K_phase = int(np.ceil(K/2))
    cvx6dof_tracker = scvx_traj_1st(quad_name, K)

    cvx_tracker_landing = lcvx_traj(quad_name, K_3dof)
    cvx6dof_tracker_landing = scvx_traj(quad_name, K)

    p_f = goal

    cvx_tracker.tf_guess_1 = 0.7*tf_guess
    cvx_tracker_landing.tf_guess_2 = 0.3*tf_guess
    # cvx_tracker.tf_guess_2 = 0.5*tf_guess

    print(cvx_tracker.tf_guess_1)
    ...
    # Start landing phase:
    print("Start landing phase!",cvx_tracker_landing.tf_guess_2)
    # cvx6dof_tracker.tf_guess = tf_guess

    X2 = cvx6dof_tracker.problem.get_variable('X') # result of 1st phase
    U2 = cvx6dof_tracker.problem.get_variable('U')

    cvx_tracker_landing.initial_p = X2[0:3, -1]
    cvx6dof_tracker_landing.initial_p = X2[0:3, -1]
    cvx_tracker_landing.initial_v = X2[3:6, -1]
    cvx6dof_tracker_landing.initial_v = X2[3:6, -1]
    cvx6dof_tracker_landing.initial_q = X2[6:10, -1]
    cvx6dof_tracker_landing.initial_w = X2[10:13, -1]
    cvx6dof_tracker_landing.initial_u = U2[:, -1]
    
    cvx_tracker_landing.final_p = p_f
    cvx6dof_tracker_landing.final_p = p_f
    cvx6dof_tracker_landing.p1 = p_f


    print('initial position:',cvx_tracker_landing.initial_p)
    cvx_tracker_landing.set_parameters()
    lcvx_t0 = time()
    cvx_tracker_landing.solve()
    print('lcvx solve time: ', time()-lcvx_t0)
    
    new_X = cvx_tracker_landing.problem.get_variable('X')
    new_U = cvx_tracker_landing.problem.get_variable('U')

    # cvx6dof_tracker.X = np.zeros(shape=[cvx6dof_tracker.m.n_x, K])
    cvx6dof_tracker_landing.X[0:6, :] = new_X

    yaw = np.zeros(shape=[1, K-K_phase+1])
    roll = np.arctan(-(new_U[1,:]/new_U[2,:]))
    pitch = np.arctan((new_U[0,:]/new_U[2,:])*np.cos(roll))
    q = euler_to_quat(yaw, roll, pitch)
    T_total = np.linalg.norm(new_U, axis=0)
    ...
    cvx6dof_tracker_landing.X[6:10, :] = q[:, 0:K]
    cvx6dof_tracker_landing.U[:, :] = np.tile(T_rotor, (4,1))
    cvx6dof_tracker_landing.sigma_2 = cvx_tracker_landing.tf_guess_2
    # cvx6dof_tracker.sigma_2 = cvx_tracker.tf_guess_2

    print('initial position:',cvx6dof_tracker_landing.initial_p)
    t_solve_start = time()

    cvx6dof_tracker_landing.solve()
    ...
    if plot:
        ...
        X3 = cvx_tracker_landing.problem.get_variable('X')
        X4 = cvx6dof_tracker_landing.problem.get_variable('X')
        
        ax.plot(X1[0, :], X1[1,:], X1[2,:], label='3-DoF')
        ax.plot(X2[0, :], X2[1,:], X2[2,:], label='6-DoF')
        ax.plot(X3[0, :], X3[1,:], X3[2,:], label='3-DoF 2nd')
        ax.plot(X4[0, :], X4[1,:], X4[2,:], label='6-DoF 2nd')
    ...
    X_traj_1 = cvx6dof_tracker.problem.get_variable('X').T
    U_traj_1 = cvx6dof_tracker.problem.get_variable('U').T
    X_traj_2 = cvx6dof_tracker_landing.problem.get_variable('X').T
    U_traj_2 = cvx6dof_tracker_landing.problem.get_variable('U').T
    X_traj = np.concatenate((X_traj_1, X_traj_2[1:, :]),axis=0)
    U_traj = np.concatenate((U_traj_1, U_traj_2[1:, :]),axis=0)

    tf_1 = cvx6dof_tracker.problem.get_variable('sigma_1')
    tf_2 = cvx6dof_tracker_landing.problem.get_variable('sigma_2')
    ...
    if replan:
        return X_traj_1, time_traj_1, U_traj_1, cvx6dof_tracker, cvx_tracker, cvx6dof_tracker_landing, cvx_tracker_landing
    else:
        return X_traj, time_traj, U_traj
    

def scvx_landing_phase_separated_replan(quad_name, goal, tf_guess, init_p, plot=False, replan=False):
    ...
    if replan:
        return X_traj, time_traj, U_traj, cvx6dof_tracker, cvx_tracker, cvx6dof_tracker_landing, cvx_tracker_landing
    else:
        return X_traj, time_traj, U_traj
