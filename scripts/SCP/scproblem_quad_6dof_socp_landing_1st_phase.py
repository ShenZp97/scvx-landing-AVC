...
class SCProblem:
    ...
    def __init__(self, m, K):
        ...
        self.var = dict()
        self.var['X'] = cvx.Variable((m.n_x, K_phase))
        self.var['U'] = cvx.Variable((m.n_u, K_phase))
        self.var['nu'] = cvx.Variable((self.n_x, K_phase - 1))
        self.var['nu_max'] = cvx.Variable((K_phase - 1,))
        # self.var['nu_max'] = cvx.Variable((1, self.K - 1))
        self.var['u_max'] = cvx.Variable((K_phase,))
        ...
        self.var['p'] = cvx.Variable((1, K_phase+1), nonneg=True)
        ...
        self.par['A_bar'] = cvx.Parameter((self.n_x * self.n_x, K_phase - 1))
        self.par['B_bar'] = cvx.Parameter((self.n_x * self.n_u, K_phase - 1))
        self.par['C_bar'] = cvx.Parameter((self.n_x * self.n_u, K_phase - 1))
        self.par['S_bar'] = cvx.Parameter((self.n_x, K_phase - 1))
        self.par['z_bar'] = cvx.Parameter((self.n_x, K_phase - 1))

        # self.par['tr_radius'] = cvx.Parameter(nonneg=True)
        self.par['X_last'] = cvx.Parameter((self.n_x, K_phase))
        ...
        #         + self.par['z_bar'][:, k]
        #         + self.var['nu'][:, k],
        #         cvx.norm(self.var['X'][0: 2, k] - self.par['final_p'][0:2], axis=0) <= (self.var['X'][2, k] - self.par['final_p'][2]) * self.tan_gamma_gs,
        ...
        small_factor = 0.5
        constraints += [cvx.norm(self.var['X'][0: 3, K_phase-1] - (self.par['final_p'][0:3] + np.array([0, 0, height])), axis=0) <= small_factor*height*np.sin(np.arctan(self.tan_gamma_gs)),
                        # -self.var['X'][2, :] <= -0.5
                        ]
        ...
        constraints += [cvx.norm(self.var['U'], 2, axis=0)[k] <= self.var['u_max'][k] for k in range(K_phase)]
