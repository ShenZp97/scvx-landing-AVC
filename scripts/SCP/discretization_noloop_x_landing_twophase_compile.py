...
def calculate_discretization(n_x, n_u, mass, x, u, tf, K) :
    # self_K = K
    # self_m = m
    self_n_x = n_x
    self_n_u = n_u
    ...
    self_A_bar = np.zeros([n_x * n_x, K - 1])
    self_B_bar = np.zeros([n_x * n_u, K - 1])
    self_C_bar = np.zeros([n_x * n_u, K - 1])
    self_S_bar = np.zeros([n_x, K - 1])
    self_z_bar = np.zeros([n_x, K - 1])
    ...
    # S_bar_end = n_x * (1 + n_x + n_u + n_u + 1)
    # z_bar_end = n_x * (1 + n_x + n_u + n_u + 2)
    # self_x_ind = slice(0, x_end)
    # self_A_bar_ind = slice(x_end, A_bar_end)
    # self_B_bar_ind = slice(A_bar_end, B_bar_end)
    # self_C_bar_ind = slice(B_bar_end, C_bar_end)
    # self_S_bar_ind = slice(C_bar_end, S_bar_end)
    # self_z_bar_ind = slice(S_bar_end, z_bar_end)

    # self_f, self_A, self_B = m.get_equations()
    ...
    A,Bm,Bp,s,z,x_prop_n = diff_discrete_foh_variational(x[0:(K-1),:],u,tf,self_dt, self_n_x, self_n_u, self_mass)
    ...
    for k in range(K - 1):
        self_A_bar[:, k] = A[k, :, :].flatten(order='F')
        self_B_bar[:, k] = Bm[k, :, :].flatten(order='F')
        self_C_bar[:, k] = Bp[k, :, :].flatten(order='F')
        self_S_bar = s.T
        self_z_bar = z.T
    return self_A_bar,self_B_bar,self_C_bar,self_S_bar,self_z_bar
...
def diff_discrete_foh_variational(x, u, tf, dt, n_x, n_u, mass):
    ...
    def dvdt(t, V, um, up, length, mass):
        ...
        A, B = diff_numeric_central(x, u, n_x, n_u, mass)
        # A, B = self.diff(x, u)
        A, B = tf * A, tf * B
    ...
    sol_y, sol_t = RK4(dvdt, (0, dtau), V0_repeat, args=(u[0:N], u[1:], N, mass), N_RK=6)

    sol = sol_y[:, -1].reshape((N, -1))
...
def forward(mass, x, u, idx=None):
    ...
    f[:, 1] = vy
    f[:, 2] = vz
    f[:, 3] = (2/mass)*(q0*q2 + q1*q3)*(u1 + u2 + u3 + u4)
    f[:, 4] = (2/mass)*(q0*q1 - q2*q3)*(-u1 - u2 - u3 - u4)
    f[:, 5] = -(1 / mass) * u1 * (2 * q1 ** 2 + 2 * q2 ** 2 - 1) - (1 / mass) * u2 * (
                2 * q1 ** 2 + 2 * q2 ** 2 - 1) - (1 / mass) * u3 * (2 * q1 ** 2 + 2 * q2 ** 2 - 1) - (
                            1 / mass) * u4 * (2 * q1 ** 2 + 2 * q2 ** 2 - 1) - 9.81
...
def diff_numeric_central(x, u, n_x, n_u, mass):
    # state & input size
    ix = n_x
    iu = n_u
    ...
    # numerical difference
    f_change_m = forward(mass, x_aug_m, u_aug_m, 0)
    f_change_p = forward(mass, x_aug_p, u_aug_p, 0)
