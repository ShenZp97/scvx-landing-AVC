...
class SCProblem:
    ...
    def __init__(self, m, K, traj_file=None):
        ...
        self.var['nu'] = cvx.Variable((self.n_x, self.K - 1))
        self.var['nu_max'] = cvx.Variable((self.K - 1,))
        # self.var['nu_max'] = cvx.Variable((1, self.K - 1))
        self.var['u_max'] = cvx.Variable((self.K,))
        ...
        self.var['p'] = cvx.Variable((1, self.K+1), nonneg=True)
        ...
        self.par['X_last'] = cvx.Parameter((self.n_x, self.K))
        ...
        #load traj for time-varying cone
        ...
        #                         2.96279158, 3.16944862, 3.37610567, 3.58276271, 3.78941975])
        start_time = 3.025
        # time_points = np.array([2.28, 2.432, 2.584, 2.736, 2.888, 3.04, 3.192, 3.344, 3.496, 3.648, 3.8])
        # start_time = 21.85-3.8

        platform_position = np.zeros(3)
        ...
        for k in range(K_phase-1, self.K - 1):
            ...
            constraints += [
                ...
                + self.par['z_bar'][:, k]
                + self.var['nu'][:, k],
                cvx.norm(self.var['X'][0: 2, k] - platform_position[0:2], axis=0) <= (self.var['X'][2, k] - platform_position[2]) * self.tan_gamma_gs,
        ...
        constraints += [self.var['sigma_1'] == self.par['sigma_last_1']]
        constraints += [self.var['sigma_2'] == self.par['sigma_last_2']]
        ...
        constraints += [cvx.norm(self.var['U'], 2, axis=0)[k] <= self.var['u_max'][k] for k in range(K)]
